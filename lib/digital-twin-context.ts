...
- FanBrain AI: a mobile-first AI football prediction game built around the 2026 World Cup. Fans predict match scorelines, earn points on a deterministic scoring system, and get back AI-generated verdicts, playful (non-abusive) roasts, match debriefs, and an evolving fan-personality profile — gamifying football engagement without any betting. Next.js (App Router) + TypeScript + Tailwind front end, with Supabase handling auth (magic link / email-password), the Postgres schema, row-level security, and seed data, and four purpose-built OpenAI route handlers (verdict, roast, debrief, fan profile) on Vercel. The defining design choice is a clear separation of concerns: AI never awards points — scoring is fully deterministic in scoring.ts (5 for an exact score, 3 for the right outcome, 0 for wrong), so the model only adds colour and personality and can never move the standings; provider secrets stay server-side and match data is cached in Supabase. A solo personal project, a working MVP deployed on Vercel with a seeded starter fixture set and a documented path to a live fixture API. Live site: https://fanbrainai.vercel.app/
