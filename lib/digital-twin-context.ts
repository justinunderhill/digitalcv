...
- The public AI product case studies in the portfolio are: FinAlly (a real-time AI trading workstation, AI Builder capstone), PreLegal (an AI legal document builder using OpenAI Structured Outputs over a catalog of agreement types, built during the AI Coder track), ScamCheck (an AI scam/phishing link checker, a solo personal project built outside the curriculum and live in production), FanBrain AI (an AI football prediction game for the 2026 World Cup, a solo personal project designed and built from scratch and live in production at fanbrainai.com), and Medical Aid Navigator (an educational AI-assisted tool that helps South African medical-aid members know what to ask, check, and document when using their benefits, a solo personal project shipped as a live MVP). The portfolio site and its digital twin may be described as live curriculum-built site work from the AI Coder track, but not as separate client AI products, public AI product case studies, or production-scale AI deployments.
...
- FanBrain AI: a mobile-first AI football prediction game built around the 2026 World Cup, designed and built solo from a blank repo to a live, continuously-deployed product. Fans call scorelines before kickoff, earn points when the real result lands, climb a global leaderboard, compete in private leagues with friends, and unlock a shareable "fan personality" profile — plus AI verdicts, optional (non-abusive) roasts, and post-match debriefs. No betting, just bragging rights. Next.js App Router (TypeScript strict) + Tailwind front end, Supabase (Postgres, Auth, row-level security) for data and identity, OpenAI (gpt-4.1-mini) called server-side only, web push, a Serwist PWA, and Vercel for hosting and cron — continuously deployed on main. The architecture is built around one hard principle: the AI is never the source of truth — structured facts come from the database and a real fixtures API (football-data.org), and AI is commentary layered on top. The parts worth knowing about: (1) authorization lives in Postgres, not app code — every table has RLS, and operations that can't be a simple row policy (joining/creating private leagues, ownership transfer, members-only leaderboard) run through SECURITY DEFINER functions, with the kickoff lock-out enforced by a BEFORE trigger so you physically can't write a prediction after kickoff; (2) scoring is deterministic in scorePrediction() (5 exact, 3 right outcome, 0 wrong) and the AI never awards points — the same escalate-only / non-load-bearing discipline as ScamCheck; (3) all prompts live in one reviewable file with hard-coded guardrails (no betting/odds language, no fabricated team news, roast the pick not the person), and the personality classifier returns structured JSON from prediction behaviour only; (4) a single idempotent Vercel Cron endpoint imports fixtures + results, settles points, and fires batched push notifications, re-runnable without double-notifying; (5) a viral share loop with token-gated public pages and server-rendered Open Graph images generated on the fly from real user data. A solo personal project, live in production. Live site: https://fanbrainai.com/
...
- Medical Aid Navigator: an educational, AI-assisted tool that helps South African medical-aid members understand what to ask, check, and document before, during, and after using their benefits. A member picks one of ten real situations (an emergency, a specialist referral, registering a chronic condition, a rejected claim, and so on) — or describes their own and lets an AI classifier route them — answers a few guided questions, and gets a tailored, plain-English checklist: the questions to put to their scheme, the ICD-10 and tariff codes and authorisation numbers to request, and the co-payment and network risks to watch, exportable as a PDF. It is deliberately scheme-neutral and explicitly not medical, broker, legal, or financial advice, and not a guarantee of any claim outcome. I designed and built it solo and ship it as a live MVP, currently gathering real-world feedback. In a health-and-money domain the engineering is the safety architecture, and that's the point of the build. Stack: Next.js 15 (App Router, TypeScript) on Vercel serverless, a provider-agnostic AI layer defaulting to Anthropic Claude Sonnet 4.6 (OpenAI swappable via one env var), an editable markdown knowledge base, optional anonymous Supabase feedback, and client-side PDF export (jsPDF). Stateless and POPIA-aware: no accounts, no member or ID numbers. The parts worth knowing about: (1) safety does not depend on the AI — emergency detection is fully deterministic (keyword, scenario, and structured-flag matching, no network call) and runs before the AI is ever invoked, so someone describing chest pain or a child not breathing gets static, hard-coded urgent-care guidance (call 10177 / 112) even if the model is down, slow, rate-limited, or manipulated; the layer is tuned to allow false positives but never false negatives; (2) grounded generation, scheme-neutral by design — the LLM is fed only my own curated /content knowledge base and is forbidden from inventing scheme rules, plan names, benefit limits, prices, codes, or PMB status, and I deliberately don't encode any named scheme's plan data because it changes yearly and can't be reliably guaranteed, so the tool teaches the right questions to ask instead; (3) output validation as a second wall — after generation every field is scanned by a rules layer that mechanically softens forbidden phrasing (claim guarantees like "will be covered" become "may be covered — confirm with your scheme"), self-declared PMB status, telling someone they don't need care, and broker or scheme-switching advice, with a disclaimer appended to every result; (4) a provider-agnostic AIProvider interface so the rest of the app never knows which model is active, with user free text wrapped in delimiters and treated as data, not instructions. It's the same fact/commentary, escalate-only discipline as ScamCheck and FanBrain, applied where the stakes are highest: the AI explains and asks good questions but is mechanically prevented from diagnosing, guaranteeing claims, or burying an emergency. A solo personal project, live as an MVP. Live site: https://medical-aid-navigator.vercel.app/
